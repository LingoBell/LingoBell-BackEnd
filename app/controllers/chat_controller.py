...
def get_recommendations(chat_room_id: int, db: Session = Depends(get_db)):
    recommendations = get_topic_recommendations_for_chat(db, chat_room_id)
    return recommendations

# AI 퀴즈생성
@router.post("/{chat_room_id}/quizzes")
def get_quiz(chat_room_id: int, db: Session = Depends(get_db), credentials: HTTPAuthorizationCredentials = Depends(security)):
    quiz = get_quiz_recommendations_for_chat(db, chat_room_id)
    return quiz
