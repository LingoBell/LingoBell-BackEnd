...
def apply_mask(img, mask):
    ...
    return: image with mask apply
    """
    # 마스크와 이미지의 크기 및 채널 수 확인
    if img.shape[:2] != mask.shape[:2]:
        mask = cv2.resize(mask, (img.shape[1], img.shape[0]))
    
    # 마스크가 1채널이 아니라면 1채널로 변환
    if len(mask.shape) == 3:
...
def warp_triangle(points_target, points_dest, cropped_triangle_target, cropped_triangle_mask_dest, rect):
    print(f"points_target shape: {points_target.shape}")
    print(f"points_dest shape: {points_dest.shape}")
    print(f"cropped_triangle_target shape: {cropped_triangle_target.shape}")
    print(f"cropped_triangle_mask_dest shape: {cropped_triangle_mask_dest.shape}")
    print(f"rect: {rect}")

    (x, y, w, h) = rect
    
    if cropped_triangle_target.size == 0 or w == 0 or h == 0:
